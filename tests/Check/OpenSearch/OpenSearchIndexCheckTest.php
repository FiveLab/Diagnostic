...
use FiveLab\Component\Diagnostic\Check\OpenSearch\OpenSearchIndexCheck;
use FiveLab\Component\Diagnostic\Result\Failure;
use FiveLab\Component\Diagnostic\Result\Success;
use FiveLab\Component\Diagnostic\Tests\Check\AbstractOpenSearchTestCase;

class OpenSearchIndexCheckTest extends AbstractOpenSearchTestCase
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        $check = new OpenSearchIndexCheck($this->getConnectionParameters(), 'test-index');

        $result = $check->check();

        self::assertEquals(new Success('Success check OpenSearch index.'), $result);
    }
    ...
    {
        $check = new OpenSearchIndexCheck(
    ...
    {
        $check = new OpenSearchIndexCheck(new OpenSearchConnectionParameters('some', 9201), 'some');
    ...
    {
        $check = new OpenSearchIndexCheck($this->getConnectionParameters(), 'some-foo');

        $result = $check->check();

        self::assertEquals(new Failure('The index was not found in OpenSearch.'), $result);
    }
